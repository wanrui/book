...
HttpSM::attach_client_session(HttpClientSession * client_vc, IOBufferReader * buffer_reader)
{
  ...
  struct timeval t;
  gettimeofday(&(t), NULL);
  t_state.client_conn_time = ((long)t.tv_sec)*1000+(long)t.tv_usec/1000;
...
HttpSM::state_raw_http_server_open(int event, void *data)
{
  ...
  case NET_EVENT_OPEN:
    ...
    //对应的配置为proxy.config.http.transaction_no_activity_timeout_out，
    netvc->set_inactivity_timeout(HRTIME_SECONDS(t_state.txn_conf->transaction_no_activity_timeout_out));
    //对应的配置为proxy.config.http.transaction_active_timeout_out，默认值为0，就是不设置超时
